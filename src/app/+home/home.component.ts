import { Headers, RequestOptions, Response, Http } from '@angular/http';
import { CurrentUserService } from '../shared/index';
import { User } from '../shared/models/user';
import { Component, OnInit } from '@angular/core';
import { AuthHttpService } from '../shared/index';


/**
 * This class represents the lazy loaded HomeComponent.
 */
@Component({
  selector: 'sd-home',
  templateUrl: 'home.component.html',
  styleUrls: ['home.component.css']
})

export class HomeComponent implements OnInit {

  newName: string = '';
  errorMessage: string;
  names: any[] = [];
  currentUser: User;

  /**
   * Creates an instance of the HomeComponent with the injected
   * NameListService.
   *
   * @param {NameListService} nameListService - The injected NameListService.
   */
  constructor(public authHttp: AuthHttpService, public http: Http, public currentUserService: CurrentUserService) {}

  dummy() {

    let headers = new Headers({ 'sdfjks': 'sdfsjm' }); // ... Set content type to JSON
    let options = new RequestOptions({ headers: headers }); // Create a request option

    this.authHttp.get('http://localhost:3000/organization_curriculum/courses' , options)
    .map((res: Response) => res.json())
    .subscribe((plans) => console.log('sdjgfs'));
  }


  /**
   * Get the names OnInit
   */
  ngOnInit() {
    this.getNames();
    this.currentUser = this.currentUserService.currentUser
  }

  /**
   * Handle the nameListService observable
   */
  getNames() {
  }

  /**
   * Pushes a new name onto the names array
   * @return {boolean} false to prevent default form submit behavior to refresh the page.
   */
  addName(): boolean {
    // TODO: implement nameListService.post
    this.names.push(this.newName);
    this.newName = '';
    return false;
  }

}
